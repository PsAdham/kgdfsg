...
setInterval(() => {
  http.get(`http://plum-bright-jumbo.glitch.me/`);
}, 280000);
...
if(message.content.startsWith(prefix + "daily")) {
  ...
  if(profile[message.author.id].lastDaily != moment().format('day')) {
   ...
   profile[message.author.id].credits += 921
    message.channel.send(`** :atm: | ${message.author.username} you received your :yen: **921**daily credits!**`)}
 else {
...
mentionned.send(` :credit_card: | Transfer Receipt \`\`\`You have received ${args[0]} from user ${message.author.username} ; (ID (${message.author.id})\`\`\``);
 ...
 client.on('message', message => {
        ...
        if(!tit) {
            message.channel.send("**:large_orange_diamond: $note <something>**");
        } else {
            ...
            ctx.drawImage(ground, 0, 0, 300, 300); // حجم الصورة
                ...
                jimp.read(url, (err, ava) => {
                    ...
                    ctx.font = 'bold 13px Comic Sans MS'; // حجم الخط و نوعه
                        ...
                        ctx.fillText(`${getvalueof.username}`, 153, 188) // احداثيات اسمك
                    ...
                    ctx.font = 'bold 13px Comic Sans MS'; // حجم الخط و نوعه
                        ...
                        ctx.fillText(`${getvalueof.username}`, 151, 185) // احداثيات اسمك
                        ...
                        ctx.textAlign = "center"; // محاذا ة النص
                        ctx.fillText(`$${profile[getvalueof.id].credits}`, 214, 147) // احداثيات المصاري
                        ...
                        ctx.fillStyle = "#f1f1f1" // لون الخط
                        ctx.textAlign = "center"; // محاذا ة النص
                        ctx.fillText(`${profile[getvalueof.id].points}`, 78, 147) // احداثيات النقاط
                        ...
                        ctx.fillStyle = "#f1f1f1" // لون الخط
                        ctx.textAlign = "center"; // محاذا ة النص
                        ctx.fillText(`${profile[getvalueof.id].level}`, 211, 88) // احداثيات اللفل
                        ...
                        ctx.fillStyle = "#000000" // لون الخط
                        ctx.textAlign = "center"; // محاذا ة النص
                        ctx.fillText(`${profile[getvalueof.id].tite}`, 150, 229) // احداثيات النقاط
                        ...
                        ctx.fillStyle = "#f1f1f1" // لون الخط
                        ctx.textAlign = "center"; // محاذا ة النص
                        ctx.fillText(`${profile[getvalueof.id].tite}`, 150, 226) // احداثيات النقاط
                        ...
                        ctx.fillStyle = "#f1f1f1";
                        ctx.textAlign = "center";
                        ctx.fillText(`+${profile[getvalueof.id].rep}`, 78,88)
...
ava.src = buf;
                        ctx.beginPath();
                        ctx.arc(75, 90, 760, 0, Math.PI*2, false);
                        ctx.closePath();
                        ctx.clip();
                          ctx.drawImage(ava, 119, 48, 47, 48);
